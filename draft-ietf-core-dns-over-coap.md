...
({{Section 6.2 of -dns-push}}; using a DNS Subscribe message)
...
({{Section 6.4 of -dns-push}}; this can involve sending an DNS Unsubscribe message or closing the session).
...
A server that can not proxy to DNS Push (or any other means of obtaining new records) SHOULD NOT send the response as a notification
